...
The focus of this $\textsf R$ Markdown script is to show basic map-making capabilities in $\textsf R$, based on Assignment 1 from Frew's ESM 263 class.  We'll use the `sf` package and `tidyverse` to:
...
For creating a pretty map layout, $\textsf R$ may be somewhat limited compared to ArcMap, especially since ArcMap has a graphical interface designed specifically for making nice layouts.  When we get to spatial analysis, we can do mamy things in $\textsf R$ that we will learn how to do in ArcMap.
