...
* In the second part we will replicate the Model Builder steps in Lab 5 by creating an Elevation layer from a topography raster clipped to our Region of Interest.
...
# Lab 5: Turning elevation raster into polygons
...
First, we'll load the raster, and then reproject it into the same coordinate system as the rest of our data.  Unfortunately, there's no good way to read the raster directly from the ArcMap geodatabase, so I exported it separately as a GeoTIFF.

```{r, eval = FALSE}
st_layers('data/Lab5.gdb')
elev_rast <- raster::raster()
```
